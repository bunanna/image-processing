...
if __name__ == "__main__":
    ...
    source = json.load(open("ix_v2.json",'r', encoding='utf-8'))
    ixp_list = source['data']
    ixp_url_stats_list = []
    ixp_name_list = []
    x = datetime.datetime.now()
    
    ixp_whitelist =['angonix',
    ...
    name = x.strftime("%Y") + '-' + x.strftime("%m") + '-' + x.strftime("%d")
    ...
    for dict_entry in ixp_list:
        if dict_entry['url_stats'] != '' and is_valid(dict_entry['website']) is not False and dict_entry['name'] in ixp_whitelist:
            try:
                ...
                main(dict_entry['url_stats'],  name + ' ' + dict_entry['name'])
            except:
                pass
